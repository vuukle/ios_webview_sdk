...
class VUEmojiVoitingView: UIView {
  
  static var sharedInstance: VUEmojiVoitingView?
  
  var emojiVotingModel = VUEmojiVotingModel(VUGlobals.requestParametes.articleID)
  ...
  override func awakeFromNib() {
    ...
    setInfo(emojiVotingModel)
    
    VUModelsFactory.requestLoadEmojiRating()
  ...
  static func instanceFromNib() -> VUEmojiVoitingView {
   
    let emojiVotingView = UINib(nibName: "VUEmojiVoitingView", bundle: Bundle(for: VUCommentsVC.self)).instantiate(withOwner: nil, options: nil)[0] as! VUEmojiVoitingView
    
    VUEmojiVoitingView.sharedInstance = emojiVotingView
    
    return emojiVotingView
  }
  ...
  private func addObserverForNewModelNotification() {
    ...
    NotificationCenter.default.addObserver(forName: VUGlobals.nEmojiUpdateModels,
                                           ...
                                           queue: nil) { [weak self] (notification) in
                                            ...
                                            exSelf.emojiVotingModel = emojiVotingModel
...
extension VUEmojiVoitingView: UICollectionViewDataSource {
...
extension VUEmojiVoitingView: VUEmojiCellDelegate {
  ...
  func didSelectEmoji(emojiCell: VUEmojiCell, button: UIButton) {
    ...
    if let emojiIndex = emojiCollectionView.indexPath(for: emojiCell),
      ...
      VUModelsFactory.voteForEmoji(emojiIndex.row,
                                   emojiVotingModel: emojiVotingModel,
                                   emojiVotingView: self)
    }
  }
}
